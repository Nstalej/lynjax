...
from app.services.connectors.base import utc_now
from app.services.connector_factory import assert_network_allowed
...
class PublicScopeRefused(DiscoveryError):
...
class DiscoveryService:
    ...
    async def _scan_host(
        ...
        if "tcp" in methods:
            ...
            open_ports.update(
                port for port, is_open in zip(REACHABILITY_PORTS, probes) if is_open
            )
        ...
        if "snmp" in methods and snmp_community:
            if await self._snmp_responds(ip, snmp_community, timeout):
                open_ports.add(161)
                if hint == "unknown":
                    hint = "snmp-capable"
    ...
    async def _snmp_responds(ip: str, community: str, timeout: float) -> bool:
        ...
        """
        from app.services.connectors import snmp_oids
        from app.services.connectors.snmp import PySnmpTransport, build_auth_data
