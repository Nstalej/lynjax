...
class DeviceSnapshot:
    ...
    def is_empty(self) -> bool:
        """True when the device returned no usable data.

        SNMP does not raise when a host is silent, it simply returns nothing.
        Without this, a device that answered not one query would be reported
        as healthy, which is the same "silence reads as success" failure the
        connectors were full of.
        """
        if self.interfaces or self.arp or self.macs or self.routes:
            return False
        # A silent SNMP agent still yields a dict, just one whose every field is
        # blank apart from a default vendor guess. That is not data.
        meaningful = {
            key: value
            for key, value in self.system_info.items()
            if key != "vendor" and str(value).strip() not in {"", "Unknown", "Generic"}
        }
    ...
    def interface(self, name: str) -> InterfaceInfo | None:
        ...
        for interface in self.interfaces:
            ...
            if (
                "/" in lowered
                and "/" in candidate
                and lowered.split("/", 1)[1] == candidate.split("/", 1)[1]
                and candidate[:2] == lowered[:2]
            ):
                return interface
        return None
